...
impl Stage {
    ...
    pub fn new_current(selector: &str) -> Option<Self> {
        ...
        if true {
            ...
            // const START: &str = "find ./ -type f -exec sed -i ";
            // const END: &str = r" {} \;";
            // I tried sed but it didn't work, so here's powershell
            /*
            $replacements = @{
                'Stage::new_current("s 17 0").unwrap()' = "Stage::from_id_current(StageID::from_components(T::Event, 17, 0)).unwrap()"
            }
            ...
            */
            // This was generated by ChatGPT but appears to work.
            // However, I do have to replace "\n\n+$" with "\n" at the end
            // workaround is I filter it to `"*.rs"` and then run `cargo fmt`
            ...
            */
            ...
            let old = format!("Stage::new_current({selector:?}).unwrap()");
            ...
            let new = {
                ...
                format!("Stage::from_id_current({stage}).unwrap()")
            };
